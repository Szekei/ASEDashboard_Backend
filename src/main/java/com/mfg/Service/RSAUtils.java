...
//        String urlPath = RSAUtils.class.getResource("/").getPath();
        ...
        System.out.println("rsa path : " + urlPath + RSA_PAIR_FILENAME);
    ...
    /** 将前台传来的已经加密好的密文以16进制编码为字符串*/

    public static String hexEncoder(String encryptedStr){
        if(StringUtils.isBlank(encryptedStr) || encryptedStr == null) {
            return null;
        }
        try {
            String hexEncryptedStr = Hex.encodeHexString(new BASE64Decoder().decodeBuffer(encryptedStr));
            throw new NoDataFoundException("ss");
//            return hexEncryptedStr;
        }catch (Exception e){
            LOGGER.error(String.format("\"%s\" Decryption failed. Cause: %s", encryptedStr, e.getMessage()));
        }
        return null;
    }

    /** 将前台传来的已经加密好的密文解密为字符串*/

    public static String decryptStringFromBase64(String encryptedStr) throws Exception {
        if (encryptedStr == null || StringUtils.isBlank(encryptedStr)){
            return null;
        }
        byte[] data = new BASE64Decoder().decodeBuffer(encryptedStr);
        PrivateKey privateKey = getKeyPair().getPrivate();
        byte[] decryptedBytes = decrypt(privateKey, data);
