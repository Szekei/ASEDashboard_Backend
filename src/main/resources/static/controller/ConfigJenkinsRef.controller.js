sap.ui.define([
    ...
    "use strict";
    var _timeout;
    return Controller.extend("ase.wt.controller.ConfigJenkinsRef", {
        ...
        handlePingJenkinsServer: function (oEvent) {
            ...
            var that = this;
            var params = that.getView().getModel().getProperty("/data/jenkinsServerRef");
        ...
        setStatusConnected: function () {
            var objectStatus = this.getView().byId("jenkinsServerRefStatus");
        ...
        handleSelectRules: function (oEvent) {
            ...
            oRulesDialog.addButton(new sap.m.Button("okButton", {
                ...
                press: function () {
                    selObj.jenkinsJobRef.rules = newObj.jenkinsJobRef.rules;
            ...
            fragModel.setData(newObj.jenkinsJobRef.rules);
        ...
        handleDeleteJob: function (oEvent) {
            ...
            if (delObj.jenkinsJobRef.jobId != 0) {
                utils.oDeletedParams.push({type: "jenkinsJobRef", id: delObj.jenkinsJobRef.jobId});
            }
            delObj.jenkinsJobRef = {};
            that.getView().getModel().refresh(true);
        },

        onToggleOpenState: function (oEvent) {
            var oTable = this.getView().byId("refJobsTable"),
        ...
        encryptPassword: function (oEvent) {
            ...
            this.getView().getModel().getData().data.jenkinsServerRef.password = encrypted;
            this.getView().getModel().refresh(true);
        ...
        handleAddJob: function (oEvent) {
            var that = this;
            var oTable = that.getView().byId("refJobsTable");
            ...
            if (!utils.validateServerInputs(dashboardData.jenkinsServerRef)) {
                return;
            }
            var selObj = oTable.getContextByIndex(sIndex).getObject();
            if (selObj.jenkinsJobRef != undefined && utils.isObjectNotNull(selObj.jenkinsJobRef.name)) {
        ...
        onChangeJob: function (oEvent) {
            ...
            if (selObj.jenkinsJobRef.jobId != 0) {
                selObj.jenkinsJobRef.op = utils._CHANGE;
            }
        },
